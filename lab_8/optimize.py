...
def get_best_model(experiment_id):
    """Obtener el mejor modelo del experimento registrado en MLflow."""
    runs = mlflow.search_runs(experiment_id)
    best_run_id = runs.sort_values("metrics.mean_valid_f1", ascending=False)["run_id"].iloc[0]
...
def objective(trial):
    ...
    # Inicializamos un nuevo run anidado para cada modelo
    with mlflow.start_run(nested=True):
        if model_type == "xgboost":
            ...
            params = {
                ...
                "max_features": trial.suggest_categorical("max_features", ["sqrt", "log2", None]),  # Aquí está el cambio
                "bootstrap": trial.suggest_categorical("bootstrap", [True, False])
            }
            model = RandomForestClassifier(**params, random_state=42)


        # Loggear los parámetros para este modelo
...
def optimize_model():
    ...
    # Crear experimento en MLflow (reiniciar o crear nuevo)
